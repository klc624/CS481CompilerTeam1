...
@Override
  ...
  {
    return "'" + deEscaping( Character.toString( c.value ) ) + "'";
  }
  ...
  public String visit( Block block )
  {
    ...
    {
      result.append( spaces(currentIndent) );
  ...
  public String visit( FunctionDefinition fun )
  {
    ...
    for ( Pair<Pair<String, Type>, Boolean> arg : fun.arguments )
    {
      ...
      var = arg.getFst().getFst();
      type = arg.getFst().getSnd().accept( this );
      ref = arg.getSnd() ? "ref " : "";
    ...
    body = fun.body.accept( this ) + spaces( currentIndent ) + "}\n";
    returnType = "";
    ...
    return "func " + fun.name + "(" + arguments + ")" + returnType
            + " {\n" + body;
  }
  ...
  public String visit( Program program )
  {
    ...
    {
      functions.append( f.accept( this ) );
      functions.append("\n");
    }
  ...
  {
    String header = "for( "
                    + forLoop.type.accept( this ) + " " + forLoop.var
                    + " in " + forLoop.collection.accept( this ) + " )";
  ...
  {
    String exp = parenthesis( stm.condition.accept( this ) );
    String block = stm.body.accept( this );

    // do while loop
    if( stm.doWhile )
    {
      return "do {\n" + spaces( currentIndent ) + block
                + spaces( currentIndent ) + "}\n" + "while " + exp;
    }

    return "while " + exp + " {\n" + block + spaces( currentIndent ) + "}";
  }
  ...
  {
    return exp.array.accept( this ) + "[ " + exp.index.accept( this ) + " ]";
  }
  ...
  public String visit( ExpAssignop exp )
  {
    if( exp.prefix )
    {
      return exp.op.toString() + " " + exp.exp.accept( this );
    }

    return exp.exp.accept( this ) + " " + exp.op.toString();
  }
}
