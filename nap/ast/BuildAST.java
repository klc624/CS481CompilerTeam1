...
public class BuildAST extends AbstractParseTreeVisitor<Ast> implements napVisitor<Ast>
{
    ...
    public Ast visitEBool(napParser.EBoolContext ctx) {
        String constant = ctx.Bool().getText();
        return new ExpBool(position(ctx), constant.equals("T"));
    }
    ...
    private Ast assignmentOperator(Position pos, napParser.ExprContext expr, Token op, boolean prefix){
        Expression exp = (Expression) visit(expr);
        OpAssign opAssign = OpAssign.INC;
        if (op.getText().equals("--")) opAssign = OpAssign.DEC;
        return new ExpAssignop(pos, opAssign, exp, prefix);
    }

    @Override
    public Ast visitEPrefix(napParser.EPrefixContext ctx) {
        Token op = ctx.AssignOp().getSymbol();
        return assignmentOperator(position(ctx), ctx.expr(), op, true);
    }

    @Override
    public Ast visitEPostfix(napParser.EPostfixContext ctx) {
        Token op = ctx.AssignOp().getSymbol();
        return assignmentOperator(position(ctx), ctx.expr(), op, false);
    }
